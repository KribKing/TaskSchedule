using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;

namespace Winning.DownLoad.Business.DTModel
{
    [Serializable]
    public class Jyinfo
    {
        public string repno { get; set; }
        public string syxh { get; set; }
        public string patid { get; set; }
        public string blh { get; set; }
        public string cardno { get; set; }
        public string hzxm { get; set; }
        public string sex { get; set; }
        public string age { get; set; }
        public string sjksdm { get; set; }
        public string sjksmc { get; set; }
        public string bqdm { get; set; }
        public string bqmc { get; set; }
        public string cwdm { get; set; }
        public string sjysdm { get; set; }
        public string sjysxm { get; set; }
        public string sjrq { get; set; }
        public string rqplb { get; set; }
        public string replbmc { get; set; }
        public string reprq { get; set; }
        public string xtbz { get; set; }
        public string jcbw { get; set; }
        public string jcysdm { get; set; }
        public string jcysxm { get; set; }
        public string jcksdm { get; set; }
        public string jcksmc { get; set; }
        public string isly { get; set; }
        public string pubtime { get; set; }
        public string yydm { get; set; }
    }
}
