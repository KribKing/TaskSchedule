...
{
    public class WnDataKsbqdyOperator : JkInterface
    {
        public WnDataKsbqdyOperator(string jkcode, string url)
            : base(jkcode, url, "", "")
        {
        ...
        public override ResultInfo Run(string jkcode, JObject jobj)
        {
            ...
            try
            {
                ...
                if (retInfo.ackflg)
                {
                    WnDataKsbqdyInfoList info = JsonConvert.DeserializeObject<WnDataKsbqdyInfoList>(retInfo.ackmsg);
                    if (info != null)
                    {
                        if (info.head.result == "success")
                        {
                            DataTable dt = Tools.ToDataTable<WnDataKsbqdyInfo>(info.body);
