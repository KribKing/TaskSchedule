...
{
    public class WnDataZgOperator : JkInterface
    {
        public WnDataZgOperator(string jkcode, string url)
            : base(jkcode, url, "", "")
        {
        ...
        public override ResultInfo Run(string jkcode, JObject jobj)
        {
            ...
            try
            {
                ...
                if (retInfo.ackflg)
                {
                    WnDataZgInfoList info = JsonConvert.DeserializeObject<WnDataZgInfoList>(retInfo.ackmsg);
                    if (info != null)
                    {
                        if (info.head.result == "success")
                        {
                            DataTable dt = Tools.ToDataTable<WnDataZgInfo>(info.body);
