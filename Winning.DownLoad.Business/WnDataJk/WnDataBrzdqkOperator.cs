...
{
    public class WnDataBrzdqkOperator : JkInterface
    {
        public WnDataBrzdqkOperator(string jkcode, string url)
            : base(jkcode, url, "", "")
        {
        ...
        public override ResultInfo Run(string jkcode, JObject jobj)
        {
            ...
            try
            {
                ...
                if (retInfo.ackflg)
                {
                    WnDataBrzdqkInfoList info = JsonConvert.DeserializeObject<WnDataBrzdqkInfoList>(retInfo.ackmsg);
                    if (info != null)
                    {
                        if (info.head.result == "success")
                        {
                            DataTable dt = Tools.ToDataTable<WnDataBrzdqkInfo>(info.body);
