...
{
    public class WnDataZddmkOperator: JkInterface
    {
        public WnDataZddmkOperator(string jkcode, string url)
            : base(jkcode, url, "", "")
        {
        ...
        public override ResultInfo Run(string jkcode, JObject jobj)
        {
            ...
            try
            {
                ...
                if (retInfo.ackflg)
                {
                    WnDataZdInfoList info = JsonConvert.DeserializeObject<WnDataZdInfoList>(retInfo.ackmsg);
                    if (info != null)
                    {
                        if (info.head.result == "success")
                        {
                            DataTable dt = Tools.ToDataTable<WnDataZdInfo>(info.body);
