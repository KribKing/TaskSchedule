...
{
    public class WnDataXmlistOperator : JkInterface
    {
        public WnDataXmlistOperator(string jkcode, string url)
            : base(jkcode, url, "", "")
        {
        ...
        public override ResultInfo Run(string jkcode, JObject jobj)
        {
            ...
            try
            {
                ...
                if (retInfo.ackflg)
                {
                    WnDataXmlistInfoList info = JsonConvert.DeserializeObject<WnDataXmlistInfoList>(retInfo.ackmsg);
                    if (info != null)
                    {
                        if (info.head.result == "success")
                        {
                            DataTable dt = Tools.ToDataTable<WnDataXmlistInfo>(info.body);
