...
{
    public class WnDataLcxmOperator : JkInterface
    {
        public WnDataLcxmOperator(string jkcode, string url)
            : base(jkcode, url, "", "")
        {
        ...
        public override ResultInfo Run(string jkcode, JObject jobj)
        {
            ...
            try
            {
                ...
                if (retInfo.ackflg)
                {
                    WnDataLcxmInfoList info = JsonConvert.DeserializeObject<WnDataLcxmInfoList>(retInfo.ackmsg);
                    if (info != null)
                    {
                        if (info.head.result == "success")
                        {
                            DataTable dt = Tools.ToDataTable<WnDataLcxmInfo>(info.body);
