	...
	public class TextEditorControl : TextEditorControlBase
	{
		...
		public string Highlighting
		{
			...
			set
			{
				...
				this.SetHighlighting(this.highlighting ?? "Default");
				this.SetFoldingStrategy(this.highlighting ?? "Default");
			}
		}
		...
		private void SetFoldingStrategy(string name)
		{
			//MessageBox.Show("设置折叠：" + name);
			Document.FoldingManager.SetFoldingStrategy(name);
		}
