...
using TaskSchedule.UI.Config;

namespace TaskSchedule
{
    public partial class NewMainFrm : FrmBase, IQuickExcuteInterface
    {
        ...
        private void Init()
        {
            try
            {
                ...
                this.SetFormPage(new ConsoleFrm(), "ConsoleFrm", "日志监控");
                Log4netUtil.IsLog = DefaultSetting.Default.islog;
                GlobalInstanceManager<JobInfoManager>.Intance = new JobInfoManager(DefaultSetting.Default.dbtype, EncodeAndDecode.Decode(DefaultSetting.Default.connstring));
        ...
        private void NewMainFrm_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        {
            this.SetFormPage(new ConfigFrm(addinfo, null, this),"addForm","新增作业");         
        }
        ...
        private void WatchConfigFrm()
        {
            ...
            if (info == null)
                return;
            this.SetFormPage(new ConfigFrm(info, node, this), info.id, info.name);

        }

        private void SetFormPage(Form frm, string id, string caption)
        {
            if (this.xtcmain.TabPages.ToList().Exists(a => a.Name == id))
            {
                this.xtcmain.SelectedTabPage = this.xtcmain.TabPages.First(a => a.Name == id);
            }
            else
            {
                frm.TopLevel = false;
                ...
                xtp.Name = id;
                xtp.Text = caption;
                xtp.Controls.Add(frm);           
                this.xtcmain.TabPages.Add(xtp);
                this.xtcmain.SelectedTabPage = xtp;
            }
        }
        ...
        {
            defaultLookAndFeel.LookAndFeel.SkinName = DefaultSetting.Default.theme;
            this.notifyIcon1.Visible = false;
            this.Text = DefaultSetting.Default.appname;
            this.Init();
        }
        ...
        private void xtcmain_CloseButtonClick(object sender, EventArgs e)
        {
            ...
            this.xtcmain.TabPages.Remove(args.Page as XtraTabPage);
        }
    }
}
