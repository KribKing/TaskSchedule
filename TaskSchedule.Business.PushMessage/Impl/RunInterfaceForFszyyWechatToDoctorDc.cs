...
{
    public class RunInterfaceForFszyyWechatToDoctorDc : RunInterface
    {
        public RunInterfaceForFszyyWechatToDoctorDc(JobInfo job) :
            base(job)
        {
        ...
        {
            string strsql = "exec usp_rims_wx_notifydoctor @cxlb=3";
            ...
            var list = dt.AsEnumerable().GroupBy(a => new { 
                ysdm = a["ysdm"].ToString(), 
                ysmc = a["ysmc"].ToString(), 
            ...
            foreach (var item in list)
            {
                ...
                string content = "您好，患者【" + item.hzxm + "】医嘱";
                msg.url = "" ;
                msg.content = content;
                msg.idNo = item.idNo;
